<template>
  <div>
    <div class="page-main">
      <div class="handleArea">
        <div class="left">
          <el-form :inline="true"
                   :model="formInline">

            <el-form-item label="操作日期：">
              <div style="display: inline-block">
                <el-date-picker v-model="formInline.startTime"
                                type="datetime"
                                placeholder="选择日期时间">
                </el-date-picker>
              </div>
              <span>-</span>
              <div style="display: inline-block">
                <el-date-picker v-model="formInline.endTime"
                                type="datetime"
                                placeholder="选择日期时间">
                </el-date-picker>
              </div>
            </el-form-item>
            <el-form-item label="用户名：">
              <el-input v-model="formInline.user"
                        placeholder="请填写用户名"></el-input>
            </el-form-item>
            <el-form-item label="操作标识：">
              <el-select v-model="formInline.region"
                         placeholder="--所有操作--">
                <el-option label="用户登录"
                           value="1"></el-option>
                <el-option label="用户退出"
                           value="2"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
              <el-button type="primary">查询</el-button>
            </el-form-item>
            <el-form-item label="">
              <el-button type="primary"
                         @click="onSubmit">导出日志</el-button>
            </el-form-item>
          </el-form>
        </div>
        <div class="right"></div>
      </div>
      <el-table :data="tableData"
                stripe
                style="width: 100%">
        <el-table-column prop="person"
                         label="操作人"
                         width="200">
        </el-table-column>
        <el-table-column prop="abstract"
                         label="操作摘要"
                         width="200">
        </el-table-column>
        <el-table-column prop="identification"
                         label="操作标识"
                         width="200">
        </el-table-column>
        <el-table-column prop="status"
                         label="操作状态"
                         width="200">
        </el-table-column>
        <el-table-column prop="time"
                         label="操作日期"
                         min-width="200">
        </el-table-column>
        <el-table-column label="操作日期"
                         width="200">
          <template slot-scope="scope">
            <span style="color:#2a6496;cursor: pointer;"
                  @click="getOperateDetail(item)">操作明细</span>
          </template>
        </el-table-column>

      </el-table>
      <pagination :total="total"
                  :page="listQuery.page"
                  :size="listQuery.size"
                  @pageChange="pageChange"></pagination>
    </div>
  </div>
</template>
<style>

</style>
<script>
import Pagination from 'components/common/pagination/index'
export default {
  name: 'administratorLog',
  components: {
    Pagination
  },
  data() {
    return {
      formInline: {
        startTime: '',
        endTime: '',
        region: '',
        user: ''
      },
      tableData: [
        {
          person: '测试组1',
          abstract: '激活设备',
          identification: '用户登录',
          status: '成功',
          time: '2018-04-12 11:17:13'
        },
        {
          person: '测试组1',
          abstract: '激活设备',
          identification: '用户登录',
          status: '成功',
          time: '2018-04-12 11:17:13'
        }
      ],
      listQuery: {
        page: 1,
        size: 8
      },
      total: 10
    }
  },
  created() {},
  mounted() {},
  computed: {},
  watch: {},
  methods: {
    onSubmit() {},
    pageChange(v) {},
    getOperateDetail(item) {
      this.$router.push({
        path: '/log/administrator/detail',
        query: { params: JSON.stringify(item) }
      })
    }
  }
}
</script>
